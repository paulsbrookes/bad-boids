...
def test_bad_boids_new_flock():
    window_data = yaml.load(
        open(os.path.join(os.path.dirname(__file__), 'fixtures', 'new_flock_fixture.yml')))
...
def test_flock_move():
    regression_data = yaml.load(
        open(os.path.join(os.path.dirname(__file__), 'fixtures', 'move_fixture.yml')))
    boid_data = np.array(regression_data["before"])
    flock = Flock()
    flock.positions = boid_data[0]
    flock.velocities = boid_data[1]
...
def test_flock_accelerate_to_middle():
    regression_data = yaml.load(
        open(os.path.join(os.path.dirname(__file__), 'fixtures', 'accelerate_to_middle_fixture.yml')))
    boid_data = np.array(regression_data["before"])
    flock = Flock()
    flock.positions = boid_data[0]
    flock.velocities = boid_data[1]
...
def test_flock_accelerate_from_boids():
    regression_data = yaml.load(
        open(os.path.join(os.path.dirname(__file__), 'fixtures', 'accelerate_from_boids_fixture.yml')))
    boid_data = np.array(regression_data["before"])
    flock = Flock()
    flock.positions = boid_data[0]
    flock.velocities = boid_data[1]
...
def test_flock_speed_match():
    regression_data = yaml.load(
        open(os.path.join(os.path.dirname(__file__), 'fixtures', 'speed_match_fixture.yml')))
    boid_data = np.array(regression_data["before"])
    flock = Flock()
    flock.positions = boid_data[0]
    flock.velocities = boid_data[1]
